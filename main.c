...
const char *argp_program_version = "grep-in-c 0.0.5-alpha";
...
static struct argp_option options[] = {
    ...
    {"case", 'a', 0, OPTION_ARG_OPTIONAL, "Case-insensitive match"},
...
void case_insensitive(FILE *f, char *pattern, bool color);
void colored_output(char *pattern);
void insensitive_colored_output(char *pattern);
  ...
  if (arguments.debug == 1) {
    ...
    printf("arguments.silent: %s\narguments.file: %s\narguments.pattern: "
           ...
           arguments.partial ? "yes" : "no", arguments.debug ? "yes" : "no", arguments.ccase ? "yes" : "no", arguments.color ? "yes" : "no");
  }
  ...
  } else if (arguments.ccase) {
    // Case-insensitive search
    case_insensitive(fptr, pattern, color);
  } else {
...
void case_insensitive(FILE *f, char *pattern, bool color) {
  while (fgets(DATA, sizeof(DATA), f) != NULL) {
    // Debug print
    // printf("[fgets()]: %s\n", DATA);
    if (strcasestr(DATA, pattern) != NULL) {
      if (color) {
        insensitive_colored_output(pattern);
      } else {
        printf("%s\n", DATA);
      }
    }
...
void insensitive_colored_output(char *pattern) {
  ...
  /* Variables needed for the colored output to work */
  char *match_start = strcasestr(DATA, pattern);
  size_t match_length = strlen(pattern);
  size_t dt_length = strlen(DATA);
  char *after_match = (match_start + match_length);
  ...
  /* Print the remaining part of the text. */
  fwrite(after_match, 1, (dt_length - (match_start - DATA) - match_length),
         stdout);
